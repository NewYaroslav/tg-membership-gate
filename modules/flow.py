...
from modules.config import ticket_categories, ticket_category_map, message_limits
...
from modules.inactivity import update_user_activity, clear_user_activity
...
async def handle_request_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(cat["label"], callback_data=f"topic:{cat['label']}")]
...
async def handle_topic_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if selected_topic not in ticket_category_map:
    ...
    cat_cfg = ticket_category_map[selected_topic]
    subcats = cat_cfg.get("subcategories") or []
    context.user_data["selected_topic"] = selected_topic
    context.user_data["selected_subtopic"] = None

    if cat_cfg.get("informational"):
        context.user_data["state"] = UserState.WAITING_FOR_TOPIC
        template_name = cat_cfg.get("template", "enter_message.txt")
        try:
            text = render_template(template_name, topic=selected_topic, subcategory=None)
    ...
    if subcats:
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(sub.get("label"), callback_data=f"subtopic:{sub.get('label')}")]
            for sub in subcats
        ])
        context.user_data["state"] = UserState.WAITING_FOR_SUBTOPIC
        text = render_template("select_subcategory.txt")
        await query.message.reply_text(text, parse_mode="HTML", reply_markup=keyboard)
        return
...
async def handle_subtopic_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query is None:
        logger.error(f"Expected CallbackQuery for subtopic, but got None (user: {user.id})")
        cat_cfg = ticket_category_map.get(selected_topic, {})
        subcats = cat_cfg.get("subcategories") or []
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(sub.get("label"), callback_data=f"subtopic:{sub.get('label')}")]
            for sub in subcats
        ])
        text = render_template("select_subcategory_intro.txt", username=username)
        await update.message.reply_text(text, parse_mode="HTML", reply_markup=keyboard)
        return

    if not query.data or not query.data.startswith("subtopic:"):
        logger.warning(f"Unexpected callback data: {query.data}")
        await query.message.reply_text("Invalid subtopic selection. Please choose one of the available options.")
        return

    selected_sub = query.data.removeprefix("subtopic:")
    cat_cfg = ticket_category_map.get(selected_topic, {})
    subcats = cat_cfg.get("subcategories") or []
    sub_label_list = [sub.get("label") for sub in subcats]

    if selected_sub not in sub_label_list:
        logger.warning(f"Invalid subtopic selected by user {user.id}: {selected_sub}")
        text = render_template("invalid_subcategory.txt")
        await query.message.reply_text(text)
        return

    sub_cfg = next((sub for sub in subcats if sub.get("label") == selected_sub), {})
    context.user_data["selected_subtopic"] = selected_sub
    context.user_data["state"] = UserState.WAITING_FOR_MESSAGE_TEXT

    template_name = sub_cfg.get("template") or cat_cfg.get("template", "enter_message.txt")
    try:
        text = render_template(template_name, topic=selected_topic, subcategory=selected_sub)
        await query.message.reply_text(text, parse_mode="HTML", reply_markup=ReplyKeyboardRemove())
    except Exception as e:
        logger.exception(f"Error in handle_subtopic_selection for user {user.id}: {e}")
        await query.message.reply_text("An unexpected error occurred. Please try again later.")
...
async def handle_text_submission(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if media_group_id:
        pending_media_groups[media_group_id].append({
            ...
            "subcategory": context.user_data.get("selected_subtopic") or "ничего"
        })
    ...
    try:
        ...
        if SUPPORT_EMAIL:
            try:
                subject = render_template("email_subject.txt", topic=topic, subcategory=subcategory)
...
async def process_media_group(entries, context):
    ...
    subcategory = entries[0].get("subcategory", "ничего")
    ...
    try:
        ...
        ud = context.application.user_data.get(telegram_id)
        if isinstance(ud, dict):
            ud["state"] = UserState.IDLE
        
        clear_user_activity(telegram_id)
