...
from modules.admin_commands import handle_document
...
async def route_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Обработка CSV-файла от администратора
    admin_state = context.user_data.get("admin_state")

    if admin_state == AdminState.WAITING_FOR_EMAIL_FILE and message.document:
        await handle_document(update, context)
        return

    # Роутинг на основании текущего состояния
    if state in (UserState.WAITING_FOR_TOPIC, UserState.WAITING_FOR_SUBTOPIC, UserState.WAITING_FOR_MESSAGE_TEXT):
    ...
    elif state == UserState.WAITING_FOR_SUBTOPIC:
...
async def handle_inline_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif query.data.startswith("subtopic:"):
        if state == UserState.WAITING_FOR_SUBTOPIC:
            await handle_subtopic_selection(update, context)
        else:
            await query.message.reply_text("An unexpected error occurred. Please try again later.")
